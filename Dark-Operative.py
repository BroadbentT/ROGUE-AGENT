      ...
      if checkParam != 1:
         ...
         with open('community.tmp', 'r') as community:
            for checkCommunity in community:
               ...
               print(colored("[*] Checking Community " + checkCommunity + "...", colour3))
               print("[+] Checking system processes...")
               remoteCOM("snmpwalk -v2c -c " + checkCommunity + " " + TIP.rstrip(" ") + " 1.3.6.1.2.1.25.1.6.0 > walk.tmp")
               catsFile("walk.tmp")      
               print("[+] Checking running processes...")
               remoteCOM("snmpwalk -v2c -c " + checkCommunity + " " + TIP.rstrip(" ") + " 1.3.6.1.2.1.25.4.2.1.2 > walk.tmp")
               catsFile("walk.tmp")      
               print("[+] Checking running systems...")
               remoteCOM("snmpwalk -v2c -c " + checkCommunity + " " + TIP.rstrip(" ") + " 1.3.6.1.2.1.25.4.2.1.4 > walk.tmp")
               catsFile("walk.tmp")      
               print("[+] Checking storage units...")
               remoteCOM("snmpwalk -v2c -c " + checkCommunity + " " + TIP.rstrip(" ") + " 1.3.6.1.2.1.25.2.3.1.4 > walk.tmp")
               catsFile("walk.tmp")      
               print("[+] Checking software names...")
               remoteCOM("snmpwalk -v2c -c " + checkCommunity + " " + TIP.rstrip(" ") + " 1.3.6.1.2.1.25.6.3.1.2 > walk.tmp")
               catsFile("walk.tmp")      
               print("[+] Checking user accounts...")
               remoteCOM("snmpwalk -v2c -c " + checkCommunity + " " + TIP.rstrip(" ") + " 1.3.6.1.4.1.77.1.2.25 > walk.tmp")
               catsFile("walk.tmp")      
               print("[+] Checking local ports...")
               remoteCOM("snmpwalk -v2c -c " + checkCommunity + " " + TIP.rstrip(" ") + " 1.3.6.1.2.1.6.13.1.3 > walk.tmp")
               catsFile("walk.tmp")      
               print("[+] Enumerating the entire MIB tree, please wait this may take sometime...")
               remoteCOM("snmpwalk -v2c -c " + checkCommunity + " "  + TIP.rstrip(" ") + " > walk.tmp")    
               print("[+] Interesting finds...")
               localCOM("grep password walk.tmp > find.tmp")
               localCOM("grep user walk.tmp >> find.tmp")
               catsFile("find.tmp")      
               prompt()
#            print("[+] Enumeration file temporary saved as walk_community.tmp for manual perusal...")            
      prompt()
